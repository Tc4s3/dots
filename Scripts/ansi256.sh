...
# Function to display colors in 5 columns with single character spacing
display_colors() {
    # Array of all hex values
    colors=(
        "#800000" "#008000" "#808000" "#000080" "#800080"
        "#008080" "#c0c0c0" "#808080" "#ff0000" "#00ff00"
        "#ffff00" "#0000ff" "#ff00ff" "#00ffff" "#ffffff"
        "#000000" "#00005f" "#000087" "#0000af" "#0000d7"
        "#0000ff" "#005f00" "#005f5f" "#005f87" "#005faf"
        "#005fd7" "#005fff" "#008700" "#00875f" "#008787"
        "#0087af" "#0087d7" "#0087ff" "#00af00" "#00af5f"
    ...
    # Print colors in 5 columns with single character spacing
    for ((i=0; i<${#colors[@]}; i+=5)); do
        for ((j=0; j<5; j++)); do
            idx=$((i + j))
            if [ $idx -lt ${#colors[@]} ]; then
                hex=${colors[$idx]}
                # Convert hex to RGB for tput
                r=$(printf "%d" "0x${hex:1:2}")
                g=$(printf "%d" "0x${hex:3:2}")
                b=$(printf "%d" "0x${hex:5:2}")
                # Set color and print with single space separator
                printf "\e[38;2;%d;%d;%dm%-7s\e[0m " $r $g $b "$hex"
            fi
        done
        printf "\n"
    done
